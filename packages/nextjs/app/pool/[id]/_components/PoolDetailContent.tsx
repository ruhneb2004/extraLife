        ...
        <div className="flex flex-col lg:flex-row gap-16 items-start">
          ...
          <div className="flex-1 max-w-2xl" style={{ fontFamily: "'Clash Display', sans-serif" }}>
            ...
            <div className="flex items-center gap-2 mb-12">
              <div className={`w-3 h-3 rounded-full ${isLive ? "bg-[#4ade80] animate-pulse" : "bg-[#f87171]"}`} />
              <span className="text-[#f87171] font-medium text-sm">
                {isLive ? pool.timeLeft : "Betting period over"}
              </span>
            </div>
            ...
            {isLive ? (
                  ...
                  <div className="flex flex-col">
                    {pool.topBets.map((bet, index) => (
                      ...
                      >
                        ...
                        />
                        <span
                          className={`text-lg ${bet.isHighest ? "font-bold text-black" : "text-gray-500 font-normal"}`}
                        >
                          {bet.isHighest
              ...
              <>
                {/* Results Section for Closed Pools */}
                <div className="mb-12">
                  <h2 className="text-2xl font-bold text-black mb-4 uppercase tracking-wide">AND THE RESULTS SAY:</h2>
                  <p
                    className={`text-7xl md:text-8xl font-black mb-4 ${pool.result?.outcome === "YES" ? "text-[#10b981]" : "text-[#f87171]"}`}
                  >
                    {pool.result?.outcome}!
                  </p>
                  <p className="text-gray-500 text-lg">{pool.result?.resultDescription}</p>
                </div>

                {/* Prize Distribution */}
                <div className="mb-8">
                  <h3 className="text-xl font-bold text-black mb-6">The prizes have been split up as:</h3>
                  <div className="space-y-4">
                    <div className="flex items-start gap-4">
                      <div className="w-4 h-4 rounded-full shrink-0 bg-[#a88ff0] mt-1" />
                      <span className="text-gray-700">
                        {pool.result?.winnerMultiplier}x of original bet for those who voted {pool.result?.outcome}.
                      </span>
                    </div>
                    <div className="flex items-start gap-4">
                      <div className="w-4 h-4 rounded-full shrink-0 bg-[#fbbf24] mt-1" />
                      <span className="text-gray-700">{pool.result?.loserMessage}</span>
                    </div>
                  </div>
                </div>
              </>
          ...
          {/* Right Side - Betting Card (only show for live pools) */}
          {isLive && (
